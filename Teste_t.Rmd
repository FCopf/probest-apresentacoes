...
errplot_jackal <- ggplot(jackal_sum,
                         aes(x = Sexo, y = media)) +
  geom_errorbar(aes(ymin = media - desvio*1.96, ymax = media + desvio*1.96), width=.1) +
  geom_point(size = 3) +
...
$s_{\overline{X}_f - \overline{X}_m} = \sqrt{\frac{`r round(desvio['Femea'],2)`}{`r n`} + \frac{`r round(desvio['Macho'],2)`}{`r n`}} = `r round(epsp,2)`$
