...
Implicações da amostragem sistemática na estimativa dos parâmetros populacionais.
___
...
.content-box-green[
<h4 style="text-align:center">Se houver um gradiente justamente na direção do transecto, a variância amostral \(s^{2}\) irá <b>superestimar</b> a variância populacional \(\sigma^2\).</h4>
...
.content-box-green[
<h4 style="text-align:center">Se houver uma periodicidade que coincida com o intervalo escolhido, a variância amostral \(s^{2}\) irá <b>subsetimar</b> a variância populacional \(\sigma^2\).</h4>
...
formas diferentes de tomarmos uma amostra de tamanho $n = `r n`$ de nossa população de tamanho $N = `r N`$.
...
$$\mu_{\overline{X}} = \sum_{i = 1}^{N}\frac{\overline{X}_i}{`r CT`} = `r mean(Allcomb)`$$
...
amostras diferentes de tamanho $n = `r n2`$ que podem ser retiradas de uma população de tamanho  $N = `r N`$. 
...
$$\mu_{\overline{X}} = \sum_{i = 1}^{N}\frac{\overline{X}_i}{`r CT2`} = `r mean(Allcomb7)`$$
