...
# 8. Intervalo de confiânça para $\hat{Y}$
...
#### A confiânça para $\hat{Y}$ aumenta ao redor de $\overline{X}$ e diminui nos extremos da distribuição de $X_i$
...
# 8. Intervalo de confiânça *vs* intervalo de predição

.pull-left[

Intervalo de confiânça de $\hat{Y}$
___
...
# 8. Intervalos de confiânça para $\hat{\beta_0}$ e $\hat{\beta_1}$
...
Fcurve <- ggplot(data = data.frame(x = c(0,4)), mapping = aes(x = x)) +
  ...
  geom_area(stat = "function", fun = df, color = 1,
                args = params,
                fill = '#d14143',
                xlim = c(lim, ylim[2])) +
  ...
  annotate(geom = 'text', x = lim - 0.3, y = 0.5, size = 8,
           color = 'gray', label = expression(F[calculado]))
...
#### Análise de resíduos: indícios de não linearidade.
...
```{r echo=FALSE, fig.width=7, fig.height=6}
plt_icy <- mmq(try_best_reg = TRUE, sem = 4)
...
```{r echo=FALSE, fig.width=7, fig.height=6}
plt_icy_res <- ggplot(plt_icy$df, aes(x = yhat, y = yres)) +
  geom_point(size = 5, shape = 19) +
  #geom_smooth(method = "loess", color = "red", linetype = 2, se = F) +
  geom_hline(yintercept = 0, color = "blue") +
  ylab(bquote("Residuo (" ~ Y - hat(Y) ~ ")")) + xlab(bquote("Estimado (" ~ hat(Y) ~ ")")) +
  theme_classic()
...
```{r echo=FALSE, fig.width=7, fig.height=6}
nl$resid <- resid(lm(Y ~ X, data = nl))
nl$fit <- predict.lm(lm(Y ~ X, data = nl))

plt_nl_res <- ggplot(nl, aes(x = fit, y = resid)) +
  geom_point(size = 5, shape = 19) +
  geom_smooth(method = "loess", color = "red", linetype = 2, se = F) +
  geom_hline(yintercept = 0, color = "blue") +
  ylab(bquote("Residuo (" ~ Y - hat(Y) ~ ")")) + xlab(bquote("Estimado (" ~ hat(Y) ~ ")")) +
...
#### Análise de resíduos: indícios de não normalidade.
...
#### Análise de resíduos: indícios de variâncias não homogêneas.
